...
from lib.helper_ui import add_select_models
from lib.helper_ui import add_select_temperature
from lib.helper_ui import add_select_top_p
...
def show_simple_generation(helper, model: str, temperature: float, max_tokens: int, top_p: float):
    ...
    # Input area
    prompt = st.text_area("Enter your prompt:", placeholder="Once upon a time...", height=100)
...
def show_creative_writing(helper, model: str, temperature: float, max_tokens: int, top_p: float):
    ...
    with col1:
        selected_template = st.selectbox("Choose Writing Template:", options=list(templates.keys()))

        topic = st.text_input("Topic/Subject:", placeholder="a mysterious library, love, adventure, etc.")
...
def show_code_generation(helper, model: str, temperature: float, max_tokens: int, top_p: float):
    ...
    with col1:
        language = st.selectbox("Programming Language:", options=list(languages.keys()))
