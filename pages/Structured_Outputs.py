...
def show_pydantic_models(helper, model: str, temperature: float):
    ...
    selected_model: type[BaseModel] = model_options[selected_model_name]
...
def show_json_schema(helper, model: str, temperature: float):
    ...
    with st.expander("Create Custom Schema"):
        custom_schema_text = st.text_area("Define your JSON schema:", value=json.dumps(selected_schema, indent=2), height=200)
...
def show_business_data(helper, model: str, temperature: float):
    ...
    business_models = {"Invoice": Invoice, "Employee Record": Employee, "Project Plan": ProjectPlan}
...
def show_creative_formats(helper, model: str, temperature: float):
    ...
    class SongLyrics(BaseModel):
        ...
        bridge: str | None = Field(description="Bridge lyrics")
...
def generate_pydantic_output(helper, model: str, prompt: str, pydantic_model: type[BaseModel] | BaseModel, temperature: float):
    """Generate structured output using Pydantic model (accepts class or instance)."""

    # Normalize to model class for display and introspection
    model_class: type[BaseModel] = pydantic_model if (isinstance(pydantic_model, type) and issubclass(pydantic_model, BaseModel)) else type(pydantic_model)

    st.markdown(f"### 🎯 Generating: {model_class.__name__}")
    ...
    with st.spinner("Generating structured output..."):
        try:
            ...
            with st.expander("✅ Validation Details"):
                st.write(f"**Model:** {model_class.__name__}")
                st.write(f"**Fields:** {len(model_class.__fields__)}")
